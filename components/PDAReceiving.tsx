...
export default function PDAReceiving() {
  ...
  // ── Normalize No. PO: strip non-digits then padStart(3,"0") ─────────────────
  // Input bisa "PO-012", "012", "12", "PO-12" — semua jadi "012"
  const normalizePO = (v: string): string => {
    const digits = v.replace(/[^0-9]/g, "");
    if (!digits) return v.trim(); // kalau tidak ada digit, kirim apa adanya
  ...
  const handleSubmit = async () => {
    ...
    // Normalize PO sebelum kirim — pastikan sama dengan format di OpenPO sheet ("012")
    const noPONormalized = normalizePO(form.no_po);
    ...
    try {
      res = await fetch("/api/sheets", {
        ...
        body: JSON.stringify({
          ...
          notes:         form.notes,
          no_po:         noPONormalized,   // ← "012" bukan "12" atau "PO-012"
      ...
      setSubmitting(false);
      setMsg({ type: "danger", text: `✗ Network error: ${fetchErr.message}` });
      return;
    }
    ...
    try {
      ...
      setSubmitting(false);
      setMsg({ type: "danger", text: "✗ Server tidak mengembalikan response yang valid" });
      return;
    }
    setSubmitting(false);
    ...
    if (json.success) {
      if (json.po_updated) {
        const slotLabel = json.slot === 1 ? "1st" : json.slot === 2 ? "2nd" : "3rd";
        setMsg({
          type: "success",
          text: `✓ GR disimpan & OpenPO diupdate · Slot ${slotLabel}${json.accumulated ? " (akumulasi)" : ""} · Total diterima: ${json.total_gr} · ${json.remark_received}`,
        });
      } else {
        // GR saved but OpenPO not updated — show warning with reason
        setMsg({
          type: "warn",
          text: `⚠ GR tersimpan di history, tapi OpenPO TIDAK diupdate: ${json.message || "Alasan tidak diketahui"}`,
        });
      }
      // Reset barcode + qty only
      setBarcodeInput("");
      setParsed(null);
      setQty("");
      loadHistory();
      ...
      setMsg({ type: "danger", text: `✗ Error: ${json.error || "Server error"}` });
    }
  };
